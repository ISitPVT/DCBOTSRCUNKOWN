...
class HelpView(discord.ui.View):
    def __init__(self, client: discord.Client, ctx: commands.Context, timeout: int):
            ...
            for command in cog.get_commands():
                if isinstance(command, commands.Group):
                    for subcommand in command.walk_commands():
                        ...
                        cmds += f"`{prefix}{command.name} {subcommand.name}`\n{emoji.bullet} {subcommand.description or 'No description provided'}\n\n"
                else:
                    prefix = self.ctx.prefix
                    cmds += f"`{prefix}{command.name}`\n{emoji.bullet} {command.description or 'No description provided'}\n\n"
...
# Help embed function to use in multiple places
def help_home_em(self, ctx: commands.Context):
...
class HelpCommands(commands.Cog, name="HelpCommands"):
    ...
    async def help(self, ctx: commands.Context):
        ...
        helpView.msg = await ctx.send(embed=help_home_em(self, ctx), view=helpView)
        
    # For processing no-prefix "help" command
    async def process_noprefix_help(self, ctx: commands.Context):
        """Process the help command when used without a prefix"""
        helpView = HelpView(self.client, ctx, timeout=60)
        helpView.msg = await ctx.send(embed=help_home_em(self, ctx), view=helpView)
